...
# Tamper sealed_payload_hash deterministically
...
$badObj = [ordered]@{
  ...
  transport          = [ordered]@{
    type                = [string]$msgObj.transport.type
    sealed_payload_hash = ("0" * 64)
  }
}
...
if($verifyText -notmatch 'VERIFY_FAIL:SEALED_PAYLOAD_HASH_MISMATCH'){
  throw ("NEG_FAIL:WRONG_FAILURE_TOKEN:`n" + $verifyText)
}

Write-Host "COURIER_NEGATIVE_SEALED_PAYLOAD_HASH_TAMPER_OK"